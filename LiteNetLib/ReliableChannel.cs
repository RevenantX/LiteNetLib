    ...
    internal sealed class ReliableChannel : BaseChannel
    {
        ...
        private NetPacket GetNextOutgoingPacket()
        {
            ...
            while (OutgoingQueue.Count > 0)
            {
                ...
                {
                    mergedPacket = Peer.NetManager.NetPacketPool.GetPacket(Peer.Mtu);
                    mergedPacket.Property = PacketProperty.ChanneledMerged;
                ...
                Peer.NetManager.NetPacketPool.Recycle(OutgoingQueue.Dequeue());
            }
        ...
        {
            if (packet.Property == PacketProperty.ChanneledMerged)
            {
                //ProcessMerged
                var pool = Peer.NetManager.NetPacketPool;
                ...
                while (pos + MergeHeaderSize <= packet.Size)
                {
                    ...
                    NetPacket mergedPacket = pool.GetPacket(NetConstants.ChanneledHeaderSize + size);
