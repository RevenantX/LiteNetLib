...
namespace LiteNetLib
{
    ...
    public class NetPeer
    {
        ...
        //Reject
        internal void Reject(long connectionId, byte connectionNumber, ReadOnlySpan<byte> data)
        {
            _connectTime = connectionId;
            _connectNum = connectionNumber;
            Shutdown(data, false);
            ...
            object userData)
        {
            ...
            if (length + headerSize > mtu)
            {
                ...
                for (ushort partIdx = 0; partIdx < totalPackets; partIdx++)
                {
                    ...
                    data.Slice(partIdx * packetDataSize).CopyTo(new Span<byte>(p.RawData, NetConstants.FragmentedHeaderTotalSize, sendLength));
                    channel.AddToQueue(p);
...
#if NETCOREAPP2_1 || NETCOREAPP3_0 || NETSTANDARD2_1
        internal bool Shutdown(ReadOnlySpan<byte> data, bool force)
        {
            int length = data.Length;
            lock (_shutdownLock)
            {
                ...
                {
                    data.CopyTo(new Span<byte>(_shutdownPacket.RawData, 9, length));
                }
                _connectionState = ConnectionState.ShutdownRequested;
                NetDebug.Write("[Peer] Send disconnect");
                NetManager.SendRaw(_shutdownPacket, EndPoint);
